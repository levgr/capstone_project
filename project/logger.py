...
def update_train_log(country,dates_range,eval_test,runtime,MODEL_VERSION,MODEL_VERSION_NOTE,test=False):
    """
    ...
    header = ['unique_id','timestamp','country','start_date','end_date','eval_test','model_version',
    ...
    with open(logfile,'a') as csvfile:
        ...
        to_write = map(str,[uuid.uuid4(),time.time(),country,dates_range[0],dates_range[1],eval_test,
...
def update_predict_log(country,y_pred,y_proba,query,runtime,MODEL_VERSION,test=False):
    ...
    ## write the data to a csv file    
    header = ['unique_id','timestamp','y_pred','y_proba','country','query','model_version','runtime']
    write_header = False
    if not os.path.exists(logfile):
        write_header = True
    with open(logfile,'a') as csvfile:
        ...
        to_write = map(str,[uuid.uuid4(),time.time(),y_pred,y_proba,country,query,
